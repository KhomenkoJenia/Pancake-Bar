!(function (e) {
	"object" == typeof module && "object" == typeof module.exports
		...
		: "undefined" != typeof jQuery && e(jQuery, window, document);
})(function (e, t, i, n) {
	!(function () {
		function t(e, t, i) {
		...
		function n() {
			if (1 === arguments.length) {
				var t = arguments[0];
				return (
					"string" == typeof t && (t = e.fn.timepicker.parseTime(t)),
					new Date(0, 0, 0, t.getHours(), t.getMinutes(), t.getSeconds())
				);
			}
			return 3 === arguments.length
				? new Date(0, 0, 0, arguments[0], arguments[1], arguments[2])
				: 2 === arguments.length
				? new Date(0, 0, 0, arguments[0], arguments[1], 0)
				: new Date(0, 0, 0);
		}
		(e.TimePicker = function () {
			...
			(e.TimePicker.prototype = {
				...
				_items: function (t, i) {
					var r,
						...
						c = null;
					for (
						-1 === t.options.timeFormat.indexOf("m") &&
							t.options.interval % 60 !== 0 &&
							(t.options.interval =
								60 * Math.max(Math.round(t.options.interval / 60), 1)),
							r = i
								? n(i)
								: t.options.startTime
								? n(t.options.startTime)
								: n(t.options.startHour, t.options.startMinutes),
							a = new Date(r.getTime() + 864e5);
						a > r;

					)
						o._isValidTime(t, r) &&
							((c = e("<li>").addClass("ui-menu-item").appendTo(s)),
							e("<a>")
								.addClass("ui-corner-all")
								.text(e.fn.timepicker.formatTime(t.options.timeFormat, r))
								.appendTo(c),
							c.data("time-value", r)),
							(r = new Date(r.getTime() + 60 * t.options.interval * 1e3));
					return s.children();
				},
				_isValidTime: function (e, t) {
					...
					return (
						...
						null !== e.options.minTime
							...
							: (null === e.options.minHour && null === e.options.minMinutes) ||
							  (i = n(e.options.minHour, e.options.minMinutes)),
						null !== e.options.maxTime
							? (r = n(e.options.maxTime))
							: (null === e.options.maxHour && null === e.options.maxMinutes) ||
							  (r = n(e.options.maxHour, e.options.maxMinutes)),
						null !== i && null !== r
							? t >= i && r >= t
							: null !== i
							? t >= i
				...
				register: function (t, i) {
					var n = this,
						r = {};
					(r.element = e(t)),
						r.element.data("TimePicker") ||
							((r.options = e.metadata
								? e.extend({}, i, r.element.metadata())
								: e.extend({}, i)),
							(r.widget = n),
							e.extend(r, {
								next: function () {
							...
							n._setDefaultTime(r),
							n._addInputEventsHandlers(r),
							r.element.data("TimePicker", r));
				},
				_setDefaultTime: function (t) {
					"now" === t.options.defaultTime
						? t.setTime(n(new Date()))
						: t.options.defaultTime && t.options.defaultTime.getFullYear
						? t.setTime(n(t.options.defaultTime))
						: t.options.defaultTime &&
						  t.setTime(e.fn.timepicker.parseTime(t.options.defaultTime));
				},
				_addInputEventsHandlers: function (t) {
					var i = this;
					t.element
						.bind("keydown.timepicker", function (e) {
				...
				activate: function (e, t) {
					...
					if (n === i.instance) {
						if ((i.deactivate(), i._hasScroll())) {
							...
							0 > r
								? i.ui.scrollTop(a + r)
								: r >= o && i.ui.scrollTop(a + r - o + t.height());
						}
						i.active = t
							.eq(0)
							.children("a")
				...
				first: function (e) {
					return this.instance === e
						? this.active && 0 === this.active.prevAll(".ui-menu-item").length
						: !1;
				},
				last: function (e) {
					return this.instance === e
						? this.active && 0 === this.active.nextAll(".ui-menu-item").length
						: !1;
				},
				selected: function (e) {
				...
				open: function (t) {
					...
					switch (
						...
						e(i).bind(
							"click.timepicker-" +
								t.element.data("timepicker-event-namespace"),
							function (e) {
								t.element.get(0) === e.target
									? t.element.data("timepicker-user-clicked-outside", !1)
									: t.element
											.data("timepicker-user-clicked-outside", !0)
											.blur();
							}
						),
						(t.rebuild || !t.items || a) &&
							(t.items = n._items(t, a ? r : null)),
						(t.rebuild || n.instance !== t || a) &&
							(e.fn.jquery < "1.4.2"
								...
								: (n.viewport.children().detach(), n.viewport.append(t.items))),
						(t.rebuild = !1),
						n.container
							.removeClass(
					...
					n.container.hasClass("ui-timepicker-no-scrollbar") ||
						t.options.scrollbar ||
					...
					var o = n.container.outerHeight() - n.container.height(),
						s = t.options.zindex
							? t.options.zindex
							: t.element.offsetParent().css("z-index"),
						c = t.element.offset();
					n.container.css({
						...
						r
							? t.items.each(function () {
									...
									return (
										(i =
											e.fn.jquery < "1.4.2"
												? e.fn.timepicker.parseTime(a.find("a").text())
												: a.data("time-value")),
										i.getTime() === r.getTime() ? (n.activate(t, a), !1) : !0
									);
							  })
							: n.deactivate(t),
						t.element
					...
					return (
						n.instance === t &&
							(n.container
								...
								.hide(),
							n.ui.scrollTop(0),
							n.ui.children().removeClass("ui-state-hover")),
						e(i).unbind(
							"click.timepicker-" + t.element.data("timepicker-event-namespace")
						),
						t.element
					);
				},
				...
				format: function (t, i, n) {
					return (
						(n = n || t.options.timeFormat), e.fn.timepicker.formatTime(n, i)
					);
				},
				getTime: function (t) {
					var i = this,
						n = e.fn.timepicker.parseTime(t.element.val());
					return n instanceof Date && !i._isValidTime(t, n)
						? null
						: n instanceof Date && t.selectedTime
						? t.format(n) === t.format(t.selectedTime)
							? t.selectedTime
							: n
						: n instanceof Date
						? n
				...
				setTime: function (t, i, r) {
					...
					if (
						...
						i && i.getMinutes && a._isValidTime(t, i))
					) {
						if (
							((i = n(i)),
							(t.selectedTime = i),
					...
					return (
						(null === o && null === t.selectedTime) ||
							(t.element.trigger("time-change", [i]),
							e.isFunction(t.options.change) &&
								t.options.change.apply(t.element, [i])),
						t.element
					);
				},
			...
			(e.fn.timepicker = function (t) {
				if ("string" == typeof t) {
					...
					return (
						(i =
							"option" === t && arguments.length > 2
								? "each"
								: -1 !== e.inArray(t, e.TimePicker.methods.chainable)
								? "each"
								: "map"),
						(n = this[i](function () {
							var i = e(this),
			...
			(e.fn.timepicker.formatTime = function (e, i) {
				var n = i.getHours(),
					...
					u = "";
				for (u in s)
					s.hasOwnProperty(u) && (c = c.replace(new RegExp(u, "g"), s[u]));
				...
				return function (r) {
					...
					return (
						...
						1 === r.length
							...
							: 2 === r.length
							? (c = r)
							: 3 === r.length || 5 === r.length
							? ((c = r.substr(0, 1)),
							  (u = r.substr(1, 2)),
							  (l = r.substr(3, 2)))
							: (4 === r.length || r.length > 5) &&
							  ((c = r.substr(0, 2)),
							  (u = r.substr(2, 2)),
							  (l = r.substr(4, 2))),
						r.length > 0 && r.length < 5 && (r.length < 3 && (u = 0), (l = 0)),
						c === !1 || u === !1 || l === !1
							? !1
							: ((c = parseInt(c, 10)),
							  (u = parseInt(u, 10)),
							  (l = parseInt(l, 10)),
							  o && 12 === c ? (c = 0) : s && 12 > c && (c += 12),
							  c > 24
									? r.length >= 6
										? e.fn.timepicker.parseTime(r.substr(0, 5))
										: e.fn.timepicker.parseTime(
												r + "0" + (o ? "a" : "") + (s ? "p" : "")
										  )
									: (a.setHours(c, u, l), a))
					);
